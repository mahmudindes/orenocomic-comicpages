...
import { addComicSynopsis, countComicSynopsis, listComicSynopsis } from '$lib/server/service';
import {
	capitalPeriod,
	formDataBoolean,
	formDataNumber,
...
import type { NewComicSynopsis } from '$lib/server/model';
...
/* export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const totalCount = await countComicSynopsis(db, paramLinks);
		const r = await listComicSynopsis(db, paramLinks);
	...
	try {
		...
		switch (request.headers.get('content-type')) {
			...
			case 'application/x-www-form-urlencoded':
				...
				v.synopsis = f.get('synonym') as string;
				v.version = f.get('version') as string;
				v.romanized = formDataBoolean(f.get('romanized'));
				break;
		}

		const r = await addComicSynopsis(db, v, a);

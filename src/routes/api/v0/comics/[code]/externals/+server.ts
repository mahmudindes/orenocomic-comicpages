...
import type { NewComicExternal } from '$lib/server/model';
...
/* export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const totalCount = await countComicExternal(db, paramLinks);
		const r = await listComicExternal(db, paramLinks);
	...
	try {
		...
		switch (request.headers.get('content-type')) {
			...
			case 'application/x-www-form-urlencoded':
				...
				v.official = formDataBoolean(f.get('official'));
				break;
		}

		const r = await addComicExternal(db, v, a);

...
import type { NewWebsite } from '$lib/server/model';
...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const totalCount = await countWebsite(db, paramLinks);
		const r = await listWebsite(db, paramLinks);
	...
	try {
		...
		switch (request.headers.get('content-type')) {
			...
			case 'application/x-www-form-urlencoded':
				...
				v.domain = f.get('domain') as string;
				v.name = f.get('name') as string;
				break;
		}

		const r = await addWebsite(db, v, a);

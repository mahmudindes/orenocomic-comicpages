...
import {
	deleteCategoryTypeByCode,
	getCategoryTypeByCode,
	updateCategoryTypeByCode
} from '$lib/server/service';
import { GenericError, NotFoundError, PermissionError } from '$lib/server/model';
import type { SetCategoryType } from '$lib/server/model';
...
export const GET: RequestHandler = async ({ params }) => {
	...
	try {
		const r = await getCategoryTypeByCode(db, params.code);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const r = await updateCategoryTypeByCode(db, params.code, v, a);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		await deleteCategoryTypeByCode(db, params.code, a);

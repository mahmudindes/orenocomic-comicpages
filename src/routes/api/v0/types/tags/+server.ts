...
import { addTagType, countTagType, listTagType } from '$lib/server/service';
...
import type { NewTagType } from '$lib/server/model';
...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const totalCount = await countTagType(db, paramLinks);
		const r = await listTagType(db, paramLinks);
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		const r = await addTagType(db, v, a);

...
import {
	deleteComicRelationTypeByCode,
	getComicRelationTypeByCode,
	updateComicRelationTypeByCode
} from '$lib/server/service';
import { GenericError, NotFoundError, PermissionError } from '$lib/server/model';
import type { SetComicRelationType } from '$lib/server/model';
...
export const GET: RequestHandler = async ({ params }) => {
	...
	try {
		const r = await getComicRelationTypeByCode(params.code);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		const r = await updateComicRelationTypeByCode(params.code, v, a);
...
export const DELETE: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		await deleteComicRelationTypeByCode(params.code, a);
